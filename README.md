...
- 스캔 PDF나 이미지 속 글자는 기본 텍스트 추출 경로와 분리합니다. Search 설정에서 선택한 VLM으로 text가 부족한 PDF page와 이미지를 OCR하며, 결과를 문서별 추출 cache에 보존합니다. 별도 유료 클라우드 OCR provider는 Codmes 기본 경로에 넣지 않습니다.
...
Codmes Search의 현재 흐름은 [검색 구조](docs/search/search-runtime.md)에 정리되어 있습니다.
PDF 필기와 텍스트/이미지 annotation을 Windows/Android까지 가져가기 위한
공통 저장 계약은 [PDF annotation 형식](docs/notes/common/pdf-annotations.md)에 정리되어 있습니다.
...
<Workspace>/.codmes/documents/    문서별 필기와 추출 상태
<Workspace>/.codmes/diffs/        패치와 diff 산출물
<Workspace>/.codmes/tool-logs/    도구 실행 기록
<Workspace>/Notes/mypage.pdf
<Workspace>/.codmes/documents/mypage--<path-hash>/annotations.json
```

문서별 필기, 추출 JSON, Markdown, OCR 이미지는 Workspace 루트의
`.codmes/documents/<readable-name--path-hash>/`에 함께 저장합니다. 폴더 안에서
`annotations.json`은 사용자 상태로, `index/`는 재생성 가능한 파생 데이터로
구분합니다. 이전 버전의 workspace/content-root/문서 옆 annotation과 해시형
추출 캐시는 처음 읽을 때 새 문서 폴더로 자동 이전됩니다.
...
- 스캔 PDF/이미지는 설정된 VLM으로 OCR할 수 있지만 안정적인 selectable bbox overlay는 아직 없습니다.
- 텍스트 레이어가 있는 PDF와 Markdown/텍스트 파일은 기존 추출 및 Workspace 검색 경로로 처리합니다.
- PDF는 Apple PDFKit 기반으로 열고, iOS/iPadOS에서는 live drawing overlay와 PDFKit `.ink` annotation으로 stroke를 즉시 표시한 뒤 workspace `.codmes/documents/<document-key>/annotations.json`에 저장합니다. 펜, 부분 지우개, lasso, 도형 자동 보정, text/image object, undo/redo, page sidebar, PDF page 삽입과 `.codmespdf` 이동을 지원합니다. 손글씨 OCR, Windows/Android editor, 정교한 layer panel, PDF 표준 annotation round-trip과 page 단위 OCR/embedding cache는 남은 작업입니다.
...
- [데이터와 저장 경로](docs/data-model.md)
- [API 계약](docs/api-contract.md)
- [Apple 클라이언트](docs/apple-client.md)
