...
- 스캔 PDF나 이미지 속 글자 OCR은 MarkItDown의 Document Intelligence/Content Understanding 같은 provider를 설정했을 때 처리할 수 있습니다. 기본 오프라인 MarkItDown PDF converter는 텍스트 레이어 추출이며, 로컬 OCR 엔진은 아닙니다.
- `tesseract`, `pdftoppm`, LibreOffice/`soffice` 같은 네이티브 도구는 공식 기본 의존성이 아닙니다.

이렇게 정리한 이유는 서버 배포 시 사용자가 별도 앱을 설치하지 않아도 Codmes가 일관되게 동작하게 만들기 위해서입니다. OCR이 필요하면 네이티브 바이너리 의존 대신 MarkItDown provider, Apple Vision, Python 라이브러리, 또는 Codmes가 직접 소유하는 OCR provider로 설계합니다. 현재는 `codmes doctor`에서 문서 처리 Python 라이브러리 준비 상태를 확인할 수 있습니다.
...
- 스캔 PDF/이미지 OCR은 기본 로컬 바이너리 없이 MarkItDown provider 경로로 열어둔 상태입니다. 텍스트 레이어가 없는 스캔 문서를 안정적으로 검색하려면 MarkItDown Document Intelligence/Content Understanding 설정 또는 향후 Codmes 소유 OCR provider가 필요합니다.
